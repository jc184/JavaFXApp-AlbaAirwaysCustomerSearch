 ...
 * @author james
 */
public class AlbaAirwaysCustomerSearchController implements Initializable {
    ...
    private void handleSearchAction() {
        ...
        if (textFieldSearch.getText().length() > 0) {
            clientTarget = client.target("http://localhost:5080/AlbaAirways-ServerSide/webresources/customer/search/{beginBy}");
            clientTarget = clientTarget.resolveTemplate("beginBy", textFieldSearch.getText());
        } else {
            clientTarget = client.target("http://localhost:5080/AlbaAirways-ServerSide/webresources/customer");
        }
    ...
    private void handleFindAction() {
        ...
        if (textFieldFind.getText().length() > 0) {
            clientTarget = client.target("http://localhost:5080/AlbaAirways-ServerSide/webresources/customer/find/{beginBy}");
            clientTarget = clientTarget.resolveTemplate("beginBy", textFieldFind.getText());
        } else {
            clientTarget = client.target("http://localhost:5080/AlbaAirways-ServerSide/webresources/customer");
        }
        GenericType<List<Customer>> listc = new GenericType<List<Customer>>() {
